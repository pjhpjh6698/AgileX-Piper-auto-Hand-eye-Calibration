...
def test_enough_poses_survive(poses):
    # 3 rows x 4 cols x 2 twists x (1 + 2 joint6 rolls) = 72 targets;
    # allow generous IK/limit drops
    assert len(poses) >= 40


def test_joint6_only_pairs_are_pure_rotation(poses):
    """Poses differing only in joint 6 must rotate without moving the flange.
    ...
    """
    qs = [np.asarray(q) for q in poses]
    pairs = [(a, b) for a, b in zip(qs, qs[1:])
             if np.allclose(a[:5], b[:5], atol=1e-9) and not np.isclose(a[5], b[5])]
    assert len(pairs) >= 20, f"only {len(pairs)} joint6-only pairs"
    for a, b in pairs:
...
def test_configured_joint6_roll_is_present(poses):
    """The +-15 deg rolls asked for actually appear in the emitted set."""
    qs = [np.asarray(q) for q in poses]
    steps = {round(math.degrees(b[5] - a[5]), 1)
             for a, b in zip(qs, qs[1:])
             if np.allclose(a[:5], b[:5], atol=1e-9)}
    for want in _Stub.serp_j6_extra_deg:
        assert any(abs(s - want) < 0.5 or abs(s - 2 * want) < 0.5 for s in steps), \
            f"no joint6 step matching {want} deg in {sorted(steps)}"
...
def test_same_stop_poses_are_rotation_rich(poses):
    """Poses sharing a lattice stop differ by rotation, not translation."""
    ...
    for a, b in zip(Ts, Ts[1:]):
        if tu.translation_distance(a, b) < 0.005:      # same lattice stop
            ...
            assert rot > 5.0, f"same-stop pose pair differs by only {rot:.1f} deg"
            pairs += 1
    assert pairs >= 20, f"only {pairs} same-stop redundant pairs survived"
