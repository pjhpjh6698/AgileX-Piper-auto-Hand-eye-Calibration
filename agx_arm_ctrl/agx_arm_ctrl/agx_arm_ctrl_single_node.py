...
class AgxArmRosNode(Node):
    ...
    def __init__(self):
        ...
        # to lower the speed per goal, and silently ignoring that is a safety
    ...
    def _on_set_parameters(self, params):
        """Apply runtime parameter changes to the SDK. LOCAL PATCH."""
        for p in params:
            if p.name != "speed_percent":
                continue
            try:
                value = int(p.value)
            except (TypeError, ValueError):
                return SetParametersResult(
                    successful=False, reason="speed_percent must be an integer")
            if not 1 <= value <= 100:
                return SetParametersResult(
                    successful=False,
                    reason=f"speed_percent must be 1..100, got {value}")
            try:
                self.agx_arm.set_speed_percent(value)
            except Exception as exc:  # noqa: BLE001
                return SetParametersResult(
                    successful=False, reason=f"set_speed_percent failed: {exc}")
            self.speed_percent = value
            self.get_logger().info(f"speed_percent -> {value}")
